...
with open("config.json", "r") as f:
    config = json.load(f)
print (config['input_video_file'])

def compress_video(video_full_path, size_upper_bound, two_pass=True, filename_suffix='cps_'):
   
    
    output_file_name = config['output_video_file']

    # Adjust them to meet your minimum requirements (in bps), or maybe this function will refuse your video!
    total_bitrate_lower_bound = config['total_bitrate_lower_bound']
    min_audio_bitrate = config['min_audio_bitrate']
    max_audio_bitrate = config['max_audio_bitrate']
    min_video_bitrate = config['min_video_bitrate']
...
if __name__ == '__main__':
    file_name = compress_video(config['input_video_file'], config['size_upper_bound'] * 1000)
    print(file_name)
